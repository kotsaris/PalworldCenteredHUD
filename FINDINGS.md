...
Resolved (2026-07-11 evening):
- Frost/cold overlay host: `WBP_IngameDamageVinette_C` (not the dispatch event receiver, which stays in keep_fullscreen as the host of other dispatched effects). Low-health vignette visually confirmed fullscreen.
...
Open:
- Compass offset example in config.lua remains available but unused; no other misplaced elements reported after extended play.
