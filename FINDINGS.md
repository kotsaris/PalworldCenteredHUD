...
- **v2.10-v2.11 — interact tooltip joins the raw-pixel family (2026-07-23)**: the building tooltip (HP "20,000/20,000", owner name, and the crafting work-in-progress readout — all one widget, `WBP_Ingame_InteractDurability_C`, 8 pooled instances) rendered a quarter-screen right of the building being looked at. Classic family signature in the F8 tree walk: direct child of the squeezed canvas, point anchors (0,0)-(0,0), game-positioned per frame. Fixed via keep_fullscreen, user-verified in-game (the crafting progress display was covered by the same fix). `WBP_InvadeDeclarationWorldHUD_PalStorage_C` (raid base-HP banner over the Pal Box, same signature, `WBP_WarningEvent_NoticeTimer_C` timer inside) was briefly added to core alongside, then REVERTED in v2.12 at the user's request — it could not be visually verified without a live raid, and unverified core entries are a liability; if the raid banner ever shows up displaced, re-add it (the anchor expansion was confirmed to apply and persist mechanically). Diagnostic lesson: it was first misidentified as the visible element; a keep_fullscreen entry that "does nothing" may simply be targeting a widget that is not currently on screen — confirm identity with an `opacity` probe (F9, no reload) before concluding the mechanism failed.
- **v2.13 — craft indicator found via child-widget paths; keep_fullscreen doubles as an exemption tool (2026-07-23 evening)**: the "work in progress" panel over a workbench job (item icon, count, assigned Pal + SAN, circular gauge) is `WBP_ConvertItemStatusIndicator_C` — the class name matched no obvious fragment (Work/Progress/Craft/Build), and it was located by reading the OUTER CHAINS of dump lines its children DID match (`...WBP_ConvertItemStatusIndicator_C_...WidgetTree...WBP_PalCraftInfo...`). Displacement confirmed caused by centering via the F6 A/B toggle test before fixing; added to core keep_fullscreen (one live instance per queued workbench job). Second finding: keep_fullscreen also serves as a per-element CENTERING EXEMPTION for statically anchored widgets — the user prefers the right-side info column (quest banner + Base Info + Base Missions, all hosted by `WBP_QuestAndBaseCampInfoCanvas_C` inside `WBP_PlayerUI_C`) at the true right screen edge, and a config `keep_fullscreen_extra` entry restores its vanilla position. Kept OUT of core deliberately: it is a preference, and core entries cannot be removed by user configs (v2.4 rule) — it ships as a commented suggestion in the example config instead. Note: this widget never appears in the F8 tree walks (walk guards skip its branch), but keep_fullscreen finds it fine — the enforcement path uses FindAllOf by class, not the walk.
...
Resolved (2026-07-23, user-verified in-game):
- Building HP/owner tooltip: `WBP_Ingame_InteractDurability_C` via keep_fullscreen (v2.11).
- Workbench craft-progress panel ("work in progress" / "being built"): `WBP_ConvertItemStatusIndicator_C` via keep_fullscreen (v2.13).
- Right-side info column at true screen edge (user preference): `WBP_QuestAndBaseCampInfoCanvas_C` via user-config `keep_fullscreen_extra`, shipped as a commented suggestion in the example config.

Open:
- Compass offset example in config.lua remains available but unused; no misplaced elements remain after extended play.
- Raid base-HP banner (`WBP_InvadeDeclarationWorldHUD_PalStorage_C`): candidate for keep_fullscreen (family signature matches, expansion applied cleanly in v2.10-v2.11) but reverted in v2.12 as unverifiable without a live raid. Revisit if it renders displaced during a raid.
